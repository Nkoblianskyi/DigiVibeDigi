                    ...
                    <div id="popup-container" style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
                        <div style={{ background: '#fff', border: '2px solid #285ca7', borderRadius: '10px', maxWidth: '400px', width: '90%', textAlign: 'center', padding: '20px', position: 'relative' }}>
                            ...
                            <div className="popup-content">
                                <Image loader={imageLoader} src="/new_spain/img/boomerang.svg" alt="Vegas Logo" width={200} height={50} style={{ marginBottom: 15 }} />
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}>
                                    {[...Array(5)].map((_, i) => <Image key={i} loader={imageLoader} src="/new_spain/img/star_icon.svg" alt="star" width={16} height={16} style={{ verticalAlign: 'middle' }} />)}
                                    <p style={{ fontWeight: 700, color: '#171717', fontSize: 24, margin: 0 }}>10.0<span style={{ fontSize: 16 }}>/10</span></p>
                                </div><br />
                                <Link href="https://boomerang-bet616356.com/de/" className="play-btn" style={{ height: 50, borderRadius: 12, background: '#8cc739', color: '#fff', fontSize: 18, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', textTransform: 'uppercase' }}>BONO COLECCIÓN</Link>
                            </div>
                        </div>
                    </div>
                )}
                ...
                <header style={{ padding: '16px 0', background: '#002222' }}>
                    <div style={{ maxWidth: 1210, margin: '0 auto', padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <Link href="/" className="logo">
                            <Image loader={imageLoader} src="/new_spain/img/logotype.svg" alt="Logo" width={170} height={50} />
                        </Link>
                    </div>
                </header>
                ...
                <div style={{ maxWidth: 1210, margin: '0 auto', padding: '0 20px' }}>
                    <div className="content" style={{ textAlign: 'center', padding: '100px 0 50px' }}>
                        ...
                        <p style={{ fontSize: 16, fontWeight: 400, maxWidth: 730, margin: '20px auto 0' }}>
                            En España hay numerosas casas de apuestas que ofrecen oportunidades seguras y atractivas para apostar. Las mejores casas de apuestas se caracterizan por plataformas fáciles de usar, cuotas lucrativas y una amplia gama de opciones de apuestas.
                        </p>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 50, gap: 50 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                <Image loader={imageLoader} className="img-icon" src="/new_spain/img/Group 1.svg" alt="icon1" width={24} height={24} />
                                <span style={{ fontSize: 14, fontWeight: 400 }}>Seguro & Fiable</span>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                <Image loader={imageLoader} className="img-icon" src="/new_spain/img/Group 2.svg" alt="icon2" width={24} height={24} />
                                <span style={{ fontSize: 14, fontWeight: 400 }}>Retiradas Rápidas</span>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                <Image loader={imageLoader} className="img-icon" src="/new_spain/img/Group 3.svg" alt="icon3" width={24} height={24} />
                    ...
                    {cards.map((card) => (
                        <div key={card.id} style={{ background: '#fff', color: '#000', borderRadius: 12, padding: 20, marginBottom: 30 }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                                <Image loader={imageLoader} src={card.logo} alt={`logo-${card.id}`} width={150} height={50} />
                                <div style={{ flex: 1 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                        {[...Array(5)].map((_, i) => (
                                            <Image key={i} loader={imageLoader} src="/new_spain/img/star_icon.svg" alt="star" width={16} height={16} />
                                        ))}
                                    ...
                                    <Link href={card.link} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', marginTop: 10, padding: '10px 20px', background: '#8cc739', color: '#fff', borderRadius: 8, textDecoration: 'none', fontWeight: 600 }}>BONO COLECCIÓN</Link>
                                </div>
                            </div>
                        </div>
                    ))}
